...
<script>
	export default {
		...
		methods: {
			async getDataAll() {
				const res = await this.$http.get(`/news/queryAll?page=1&size=9`);
				console.log(res);
			...
			async changeData(index){
				...
				const res = await this.$http.get(`/news/queryAll?page=${this.currentPage}&size=9`);
				console.log(res);
				this.contentArr = res.data.data.list;
			},
			//搜索标题
			async searchTilte(){
				...
				const res = await this.$http.get(`/news/queryTitle?title=${this.title}`);
				console.log(res);
				this.contentArr = res.data.data.list;
			},
			toDetail(index) {
				...
				if (str === 'admin') {
					this.$router.push({
						path: `/adminHome/counsel/detail/${this.contentArr[index].id}`,
					})
				} else {
					this.$router.push({
						path: `/userHome/counsel/detail/${this.contentArr[index].id}`,
					})
				}
			}
		}
