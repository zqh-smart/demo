		...
		<div class="foo-top">
			...
			<div class="info-right">
				<el-form :model="infoForm" ref="infoForm" :rules="rules" label-position="left" label-width="60px"
					...
					<el-form-item label="密码" v-show="isDisabled" class="form-item" prop="password">
		...
		<el-container class='foo-main'>
			<el-aside width="300px">
				<el-menu :default-active="defaultActive" class="foo-nav" :router="true" background-color=" #e5edf0"
					...
					<el-menu-item index="/userHome/center/warnMessage" style="border-bottom: none" class="item">预警消息
					</el-menu-item>
					<el-menu-item index="/userHome/center/collection" class="item">我的收藏</el-menu-item>
					<el-menu-item index="/userHome/center/apply" style="border-bottom: 1px solid #fff;" class="item">
						返乡申请</el-menu-item>
				</el-menu>
			</el-aside>
	...
	export default {
		data() {
			return {
				...
				avatarArr: [],
				title: '预警消息',
				defaultActive: '/userHome/center/warnMessage',
				currentPath: '',
				role: '普通用户',
				isDisabled: true,
				infoForm: {
		...
		mounted() {
			...
			if (this.$route.path != '/userHome/center/warnMessage') {
				this.$router.replace({
					name: 'warnMessage'
				})
			}
		...
		methods: {
			...
			initInfoForm() {
				...
				this.infoForm.password = this.$md5(info.password);
			...
			save(formName) {
				if (this.isDisabled) {
					...
					// console.log(this.infoForm.Newpassword)
			...
			changeTitle(key, path) {
				if (key === '/userHome/center/warnMessage') {
					this.title = '预警消息'
				} else if (key === '/userHome/center/collection') {
					this.title = '我的收藏'
				} else {
					this.title = '返乡申请'
				}
